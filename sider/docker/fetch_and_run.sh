...
# Configure ssmtp if SSMTP_CONFIG_S3_URL is defined
SSMTP_CONFIG_PATH="/etc/ssmtp/ssmtp.conf"
if [[ -n "$SSMTP_CONFIG_S3_URL" ]]; then
	aws s3 cp "$SSMTP_CONFIG_S3_URL" "$SSMTP_CONFIG_PATH" || error_exit "Failed to download ssmtp.conf from $SSMTP_CONFIG_S3_URL"
	echo "hostname=$HOSTNAME" >> "$SSMTP_CONFIG_PATH"
fi
